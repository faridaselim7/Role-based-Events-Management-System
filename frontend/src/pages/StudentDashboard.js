...
import { Calendar, Clock, User, MapPin, XCircle, Star, Heart, Store, Package } from "lucide-react";
...
import { 
  ...
  NoSearchResultsState,   EmptyState 
...
export const StudentDashboard = ({ user, onLogout }) => {
  ...
  const navigation = [
    ...
    { name: "Courts", icon: <MapPin className="w-4 h-4" />, view: "courts" },
    { 
      name: "Polls", 
      view: "polls",
  ...
  useAllEventNotifications("student");
  ...
  useEffect(() => {
    const fetchRegisteredEvents = async () => {
      try {
        ...
        if (!token) {
          console.warn("No token found for reminders");
          return;
        }
        ...
        if (!res.ok) {
          ...
          console.error("Failed to fetch registrations:", res.status, error);
          return;
        }
        ...
        const now = Date.now();
        const scheduled = new Set(); // Prevent duplicates
        ...
        registeredEvents.forEach((event) => {
          ...
          const reminders = [
            { delay: eventTime - 24 * 60 * 60 * 1000, id: `${event._id}-1d`, msg: "starts tomorrow!" },
            { delay: eventTime - 60 * 60 * 1000,       id: `${event._id}-1h`, msg: "starts in 1 hour!" },
          ];
  
          reminders.forEach(({ delay, id, msg }) => {
            const timeUntil = delay - now;
  
            if (timeUntil > 0 && timeUntil < 30 * 24 * 60 * 60 * 1000 && !scheduled.has(id)) { // < 30 days
              scheduled.add(id);
              ...
              setTimeout(() => {
                addNotification({
                  id,
                  type: "info",
                  message: `Reminder: "${event.title}" ${msg}`,
                  read: false,
                  temporary: false,
...
const fetchProfessorNames = async (ids) => {
  ...
  return responses.filter(Boolean); // remove any nulls
};
...
// Unified VendorsSection with Voting
...
export { EventsView, FavoritesView };