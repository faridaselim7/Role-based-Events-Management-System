...
const EventFilterSort = ({ 
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (filters.professorName) {
        result = result.filter(e => getProfessor(e) === filters.professorName);
      }
  ...
  const hasActiveFilters = filters.name || filters.location || filters.type || 
                          filters.dateRange.start || filters.dateRange.end || filters.professorName ||
                          sortBy !== 'date-asc';
      ...
      {showFilters && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 p-4 bg-[#F8FAFC] rounded-lg">
          ...
          <div>
            ...
            <div>
  <label className="block text-xs font-medium text-[#2B4B3E] mb-2">Professor Name</label>
  <select
